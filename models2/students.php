...
class Student {
    ...
    public static function get($matricula) {
        ...
        if ($command->fetch()) {
            return new Student($matricula, $nombre, $apePat, $apeMat, $promedio, $correo, $telefono, $genero, $fechaNacimiento, $curp, $foto);
        } else {
            return null;
        }
    }
    ...
    public static function searchStudent($searchTerm, $limit = 0, $offset = 0) {
        if (!self::$conn) {
            die("Error: No se ha establecido la conexión a la base de datos.");
        }
