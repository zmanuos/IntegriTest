...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_check_telefono_docente->num_rows > 0 || $result_check_telefono_alumno->num_rows > 0) {
        echo json_encode(['success' => false, 'error' => 'El teléfono ya está registrado en el sistema.']);
        exit();
    }

    $sql_insert = "INSERT INTO alumno (nombre, apellidoP, apellidoM, telefono, genero, fechaNacimiento, curp) 
                VALUES ('$nombre', '$apellidoP', '$apellidoM', '$telefono', '$genero', '$fechaNacimiento', '$curp')";

    if ($conn->query($sql_insert) === TRUE) {
        $sql_matricula = "SELECT matricula, foto, correo FROM alumno WHERE curp = '$curp'";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
                ...
                list($originalWidth, $originalHeight) = getimagesize($fotoTmpPath);
                $image = ($fotoType === 'image/jpeg' || $fotoType === 'image/jpg') ? imagecreatefromjpeg($fotoTmpPath) : imagecreatefrompng($fotoTmpPath);
            ...
            echo json_encode(['success' => false, 'error' => 'Error al obtener los datos del nuevo alumno.']);
        }
    } else {
        echo json_encode(['success' => false, 'error' => 'Error al registrar el alumno.']);
    }
}
?>